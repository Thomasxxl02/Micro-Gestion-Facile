...
#### Régénération complète (après exposition)

Si une clé a été exposée/compromise:

**Pour Firebase API Key:**

1. Accédez à https://console.cloud.google.com/apis/credentials
2. Sélectionnez le projet "micro-gestion-facile"
3. Onglet **"API Keys"** → Trouvez la clé
4. Cliquez **"Delete"** pour révoquer
5. Bouton **"+ CREATE CREDENTIALS"** → **"API Key"**
6. Configurez les restrictions :
   - **Application restrictions**: `HTTP referrers`
   - Domaines: `https://micro-gestion-facile.vercel.app`, `http://localhost:*`
   - **API restrictions**: Cloud Firestore API, Firebase App Check API
7. Sauvegardez dans `.env.local` et GitHub Secrets

**Pour Gemini API Key:**

1. Allez à https://aistudio.google.com/apikey
2. Trouvez l'ancienne clé et cliquez **"Delete"**
3. Cliquez **"+ Create API Key"**
4. Sélectionnez le projet Firebase
5. Activez **"API restrictions"** → Generative Language API
6. Sauvegardez la nouvelle clé et mettez à jour

**Pour GitHub OAuth Secrets:**

1. Allez à https://github.com/Thomasxxl02/Micro-Gestion-Facile/settings/secrets/actions
2. Mettez à jour `VITE_FIREBASE_API_KEY` et `GEMINI_API_KEY`
3. Testez: `git commit --allow-empty -m "test: validate new API keys"` et `git push`
...
### ⚠️ En cas d'exposition de clé

1. **Révoquez immédiatement** la clé compromise
2. **Générez une nouvelle clé** avec les mêmes restrictions
3. **Mettez à jour** `.env.local` et GitHub Secrets
4. **Déclenchez un nouveau build** pour valider
5. **Vérifiez les logs** pour détecter un accès non autorisé
