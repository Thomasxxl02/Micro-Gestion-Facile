...
const getStatusColorClass = (status: InvoiceStatus): string => {
  if (status === InvoiceStatus.PAID || status === InvoiceStatus.ACCEPTED) {
...
const getStatusBadgeClass = (status: InvoiceStatus): string => {
  if (status === InvoiceStatus.PAID || status === InvoiceStatus.ACCEPTED) {
    return 'bg-accent-50 dark:bg-accent-900/20 text-accent-600 dark:text-accent-400 border-accent-100 dark:border-accent-900/50';
  }
  if (status === InvoiceStatus.SENT) {
  ...
  return 'bg-brand-50 dark:bg-brand-800 text-brand-500 dark:text-brand-400 border-brand-200 dark:border-brand-700';
};
...
const InvoiceManager: React.FC<InvoiceManagerProps> = ({
  ...
  // Convenient aliases for existing code compatibility
  const newDocData = form.formData;
  const setNewDocData = (
    data: Partial<typeof newDocData> | ((prev: typeof newDocData) => Partial<typeof newDocData>)
  ) => {
    const newData = typeof data === 'function' ? data(newDocData) : data;
    form.setFormData(newData);
  };
  const selectedClientId = newDocData.clientId;
  const setSelectedClientId = (id: string) => form.setFormData({ clientId: id });

  // UI state for modal & generation features
  ...
  const isFormValid = useCallback(() => {
    return !!(
      selectedClientId &&
      newDocData.items?.length &&
      newDocData.date &&
      newDocData.dueDate
    );
  }, [selectedClientId, newDocData.items, newDocData.date, newDocData.dueDate]);
  ...
  }, [form, userProfile.defaultVatRate]);
  ...
  const addProductItem = useCallback(
    (productId: string) => {
      ...
      if (product.type === 'product' && product.stock !== undefined && product.stock <= 0) {
        if (
          !confirm(
            `Attention : Le produit "${product.name}" est en rupture de stock. Voulez-vous quand même l'ajouter ?`
          )
        ) {
          return;
        }
    ...
    [form, products, userProfile.defaultVatRate]
  );

  const updateItem = useCallback(
    (id: string, field: keyof InvoiceItem, value: string | number) => {
      const index = newDocData.items?.findIndex((item) => item.id === id) ?? -1;
      if (index !== -1) {
        form.updateItem(index, { [field]: value });
      }
    },
    [form, newDocData.items]
  );
  ...
  const handleMagicFill = async () => {
    if (!formState.magicFillPrompt.trim()) {
      return;
    }
    ...
    if (items && items.length > 0) {
      const formattedItems = items.map(
        (item: { description: string; quantity: number; unitPrice: number; unit: string }) => ({
          ...item,
  ...
  const convertToInvoice = (doc: Invoice, isQuote: boolean) => {
    ...
    if (isQuote && doc.status !== InvoiceStatus.ACCEPTED) {
      updatedInvoices = invoices.map((i) =>
        i.id === doc.id ? { ...i, status: InvoiceStatus.ACCEPTED } : i
      );
    }
    ...
    const newInvoice: Invoice = {
      ...
      status: InvoiceStatus.DRAFT,
      notes: isQuote
        ? `Facture suite au devis ${doc.number}`
        : `Facture pour la commande ${doc.number}`,
    };
  ...
  const updateInvoiceField = (
    id: string,
    field: 'status' | 'reminderDate',
    value: InvoiceStatus | string
  ) => {
  ...
  const handleTransmitPPF = (invoice: Invoice) => {
    if (
      !confirm(
        `Transmettre la facture ${invoice.number} au Portail Public de Facturation (PPF) au format ${invoice.eInvoiceFormat || 'Factur-X'} ?`
      )
    ) {
      return;
    }
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <button
              ...
              className="p-2.5 hover:bg-brand-200 text-brand-500 rounded-xl transition-all"
            ...
            <div>
              <h2 className="text-2xl font-bold text-brand-900 font-display">
                Éditeur : {getDocumentLabel(activeTab)}
              </h2>
            ...
            <div className="bg-white rounded-4xl shadow-sm border border-brand-200 p-8">
              {newDocData.linkedDocumentId && (
                <div className="mb-6 inline-flex items-center gap-2 bg-brand-100 text-brand-600 px-3 py-1.5 rounded-full text-xs font-semibold">
                  <LinkIcon size={12} />
                  Lié au document ID:{' '}
                  {invoices.find((i) => i.id === newDocData.linkedDocumentId)?.number || 'Inconnu'}
                </div>
              )}
                ...
                <div>
                  <Combobox
                    options={clients
                      ...
                      .map((c) => ({
                        id: c.id,
                        label: c.name,
                        subLabel: c.email || c.siret || 'Client',
                      }))}
                    ...
                    onChange={(clientId) => {
                      ...
                      if (client?.paymentTerms) {
                        let days = 30;
                        if (client.paymentTerms === 'À réception') {
                          days = 0;
                        } else if (client.paymentTerms === '15 jours') {
                          days = 15;
                        } else if (client.paymentTerms === '45 jours') {
                          days = 45;
                        } else if (client.paymentTerms === '60 jours') {
                          days = 60;
                        }
                  ...
                  {selectedClient && (
                    <div className="mt-4 p-4 bg-brand-50 rounded-xl border border-brand-100 text-sm">
                      <p className="font-bold text-brand-800 font-display">{selectedClient.name}</p>
                      <p className="text-brand-500 whitespace-pre-line mt-1">
                        {selectedClient.address}
                      </p>
                      {selectedClient.siret && (
                        <p className="text-xs text-brand-400 mt-2 font-mono">
            ...
            <div className="bg-white rounded-4xl shadow-sm border border-brand-200 p-8">
              ...
              <div className="space-y-4">
                {newDocData.items?.map((item, index) => (
                  <div
                    key={item.id}
                    className="flex flex-col md:flex-row gap-4 items-start md:items-center bg-brand-50/50 p-4 rounded-xl border border-brand-100 group hover:border-accent-200 transition-colors"
                  >
                    <span className="bg-white w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-brand-400 border border-brand-200 shadow-sm shrink-0">
                      {index + 1}
                    </span>
                    <div className="flex-1 w-full relative">
                      ...
                      >
                        <Wand2
                          size={14}
                          className={formState.isGeneratingDesc ? 'animate-spin' : ''}
                        />
                      </button>
                    </div>
                    <div className="flex gap-4 w-full md:w-auto">
                      <div className="flex flex-col w-20">
                        ...
                        <input
                          ...
                          value={item.quantity}
                          onChange={(e) =>
                            updateItem(item.id, 'quantity', Number.parseFloat(e.target.value))
                          }
                          title="Quantité"
                        />
                      </div>
                      ...
                      <div className="flex flex-col w-24">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateItem(item.id, 'unitPrice', Number.parseFloat(e.target.value))
                          }
                          title="Prix unitaire"
                        />
                      </div>
                      ...
                      <div className="flex flex-col w-24 justify-end">
                        <div className="text-right font-bold text-brand-900 py-2 font-display">
                          {(item.quantity * item.unitPrice).toFixed(2)} €
                        </div>
                      </div>
                      <button
                        onClick={() => removeItem(item.id)}
                        className="self-end mb-2 text-brand-300 hover:text-red-500 transition-colors"
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <button
                    onClick={addItem}
                    className={`flex-1 py-3 border-2 border-dashed border-brand-200 rounded-xl text-brand-500 hover:border-brand-400 hover:text-brand-600 transition-all font-bold text-[10px] uppercase tracking-widest flex items-center justify-center gap-2`}
                    ...
                    <Combobox
                      options={products
                        ...
                        .map((p) => ({
                          id: p.id,
                          label: p.name,
                          subLabel: `${p.price.toFixed(2)}€ | ${p.category || 'Sans catégorie'}`,
                        }))}
                      value=""
                      onChange={(productId) => {
                        if (productId) {
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewDocData({
                        ...newDocData,
                        operationCategory: e.target.value as 'BIENS' | 'SERVICES' | 'MIXTE',
                      })
                    }
                  ...
                  <p className="mt-2 text-[10px] text-brand-400 italic">
                    Obligatoire pour la transmission PPF/PDP en 2026.
                  </p>
                </div>
                <div>
                  <label
                    htmlFor="einvoice-format-select"
                    className="block text-xs font-bold text-brand-500 uppercase tracking-wider mb-2"
                  >
                    Format d&apos;export cible
                  </label>
                  <select
                    ...
                    onChange={(e) =>
                      setNewDocData({
                        ...newDocData,
                        eInvoiceFormat: e.target.value as 'Factur-X' | 'UBL' | 'CII',
                      })
                    }
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-brand-500 leading-relaxed">
                      En activant ces options, vos factures incluront les métadonnées nécessaires
                      pour être acceptées par le Portail Public de Facturation (PPF) ou votre
              ...
              <div className="space-y-4 mb-6 text-sm">
                <div className="flex justify-between items-center text-brand-300">
                  ...
                  <span className="font-bold font-display">
                    {formTotals.subtotalHT.toFixed(2)} €
                  </span>
                </div>

                {/* Tax Exempt Toggle */}
                <div className="flex justify-between items-center py-2 border-y border-white/5">
                  <span className="text-xs text-brand-400 font-medium italic">
                    Exonéré de TVA (Auto-entrepreneur)
                  </span>
                  <button
                    onClick={() =>
                      setNewDocData({ ...newDocData, taxExempt: !newDocData.taxExempt })
                    }
                  ...
                  >
                    <div
                      className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${newDocData.taxExempt ? 'left-6' : 'left-1'}`}
                    />
                  </button>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewDocData({ ...newDocData, discount: Number.parseFloat(e.target.value) })
                    }
                    placeholder="0"
                ...
                <div className="flex justify-between items-center">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewDocData({ ...newDocData, shipping: Number.parseFloat(e.target.value) })
                    }
                    placeholder="0.00"
                ...
                <div className="bg-white/5 p-4 rounded-xl mt-6">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewDocData({ ...newDocData, deposit: Number.parseFloat(e.target.value) })
                      }
                      placeholder="0.00"
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-white/10">
                    ...
                    <span className="text-accent-200 font-bold text-lg font-display">
                      {formTotals.balanceDue.toFixed(2)} €
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-brand-50 w-full max-w-5xl h-[90vh] rounded-4xl flex flex-col shadow-2xl overflow-hidden relative border border-brand-200">
              ...
              <div className="flex-1 overflow-y-auto p-8 flex justify-center bg-brand-100/50">
                <InvoicePaper
                  ...
                  clients={clients}
                  isPreview={true}
                  onLinkedDocumentClick={openLinkedDocument}
                />
              </div>
            </div>
          </div>
  ...
  if (view === 'detail' && selectedInvoice) {
    ...
    const parentDoc = selectedInvoice.linkedDocumentId
      ? invoices.find((inv) => inv.id === selectedInvoice.linkedDocumentId)
      : null;
    ...
    return (
      <div className="max-w-4xl mx-auto animate-fade-in pb-10">
        <div className="sticky top-0 z-10 bg-brand-50/80 backdrop-blur-md py-4 mb-6 flex justify-between items-center print:hidden border-b border-brand-200/50 no-print">
          <button
            onClick={() => setView('list')}
            className="flex items-center gap-2 text-brand-600 hover:text-brand-900 font-bold px-4 py-2 hover:bg-white rounded-xl transition-all uppercase tracking-widest text-[10px]"
          >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 bg-white border border-brand-200 rounded-xl p-1 pr-3 shadow-sm">
              <span className="text-[10px] font-bold text-brand-400 uppercase tracking-widest ml-3">
                Statut
              </span>
              {isCustomStatus ? (
                <div className="flex items-center gap-1">
                  <input
                    ...
                    onChange={(e) =>
                      updateInvoiceField(selectedInvoice.id, 'status', e.target.value)
                    }
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateInvoiceField(
                      selectedInvoice.id,
                      'status',
                      e.target.value as InvoiceStatus | 'CUSTOM_INPUT'
                    )
                  }
                ...
                >
                  ...
                  <option value="CUSTOM_INPUT" className="font-bold text-brand-600">
                    + Personnalisé...
                  </option>
                </select>
              )}
            ...
            {selectedInvoice.status !== InvoiceStatus.PAID && (
              <div className="flex items-center bg-white border border-brand-200 rounded-xl shadow-sm relative group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateInvoiceField(selectedInvoice.id, 'reminderDate', e.target.value)
                  }
            ...
            {docType === 'quote' && selectedInvoice.status === InvoiceStatus.SENT && (
              <>
                <button
                  onClick={() =>
                    updateInvoiceField(selectedInvoice.id, 'status', InvoiceStatus.ACCEPTED)
                  }
                ...
                <button
                  onClick={() =>
                    updateInvoiceField(selectedInvoice.id, 'status', InvoiceStatus.REJECTED)
                  }
        ...
        {allLinkedDocs.length > 0 && (
          <div className="mb-8 flex flex-wrap gap-4 no-print">
            {allLinkedDocs.map(({ doc, relation }) => (
              <div
                key={doc.id}
                className="flex-1 min-w-75 bg-white border border-brand-100 rounded-3xl p-5 flex items-center justify-between shadow-sm hover:shadow-md transition-all"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-brand-900 font-display">
                      ...
                      <span className="text-brand-500">#{doc.number}</span>
                    </p>
                  </div>
                </div>
                <button
    ...
    <div className="space-y-8 max-w-7xl mx-auto animate-fade-in relative pb-12">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          ...
          <p className="text-brand-500 dark:text-brand-400 mt-1 text-sm font-medium">
            Gérez vos factures, devis et commandes en un seul endroit.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="bg-brand-100/50 dark:bg-brand-900/50 p-1 rounded-2xl border border-brand-100 dark:border-brand-800 backdrop-blur-sm flex gap-1">
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all duration-300 flex items-center gap-2 ${activeTab === 'invoice' ? 'bg-white dark:bg-brand-800 text-brand-900 dark:text-brand-50 shadow-sm' : 'text-brand-400 dark:text-brand-500 hover:text-brand-600 dark:hover:text-brand-300'}`}
            >
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all duration-300 flex items-center gap-2 ${activeTab === 'quote' ? 'bg-white dark:bg-brand-800 text-brand-900 dark:text-brand-50 shadow-sm' : 'text-brand-400 dark:text-brand-500 hover:text-brand-600 dark:hover:text-brand-300'}`}
            >
              <FileCheck size={14} /> Devis
            </button>
            <button
              onClick={() => {
                setActiveTab('order');
              ...
              className={`px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all duration-300 flex items-center gap-2 ${activeTab === 'order' ? 'bg-white dark:bg-brand-800 text-brand-900 dark:text-brand-50 shadow-sm' : 'text-brand-400 dark:text-brand-500 hover:text-brand-600 dark:hover:text-brand-300'}`}
            >
            ...
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all duration-300 flex items-center gap-2 ${activeTab === 'credit_note' ? 'bg-white dark:bg-brand-800 text-brand-900 dark:text-brand-50 shadow-sm' : 'text-brand-400 dark:text-brand-500 hover:text-brand-600 dark:hover:text-brand-300'}`}
            >
              <Receipt size={14} /> Avoirs
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="card-modern p-6 flex flex-col justify-between relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="badge bg-brand-50 dark:bg-brand-800 text-brand-600 dark:text-brand-400">
              Total {getDocumentLabel(activeTab)}s
            </span>
          </div>
          <div>
            <h3 className="text-2xl font-bold text-brand-900 dark:text-white font-display tracking-tight">
            ...
            <p className="text-xs text-brand-500 dark:text-brand-400 mt-1 font-medium">
              {stats.count} documents au total
            </p>
          </div>
        </div>
        <div className="card-modern p-6 flex flex-col justify-between relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start mb-4">
            ...
            <span className="badge bg-accent-50 dark:bg-accent-900/20 text-accent-600 dark:text-accent-400">
              Réglé / Accepté
            </span>
          </div>
          <div>
            <h3 className="text-2xl font-bold text-accent-600 dark:text-accent-400 font-display tracking-tight">
              {stats.paid.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
            </h3>
            <p className="text-xs text-brand-500 dark:text-brand-400 mt-1 font-medium">
              Revenus sécurisés
            </p>
        ...
        <div className="card-modern p-6 flex flex-col justify-between relative overflow-hidden group">
          ...
          <div>
            <h3 className="text-2xl font-bold text-amber-600 dark:text-amber-400 font-display tracking-tight">
              {stats.pending.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
            </h3>
            <p className="text-xs text-brand-500 dark:text-brand-400 mt-1 font-medium">
              À encaisser
            </p>
        ...
        <div className="card-modern p-6 flex flex-col justify-between relative overflow-hidden group border-red-100 dark:border-red-900/30">
          ...
          <div>
            <h3 className="text-2xl font-bold text-red-600 dark:text-red-400 font-display tracking-tight">
              {stats.overdue.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
            </h3>
            <p className="text-xs text-brand-500 dark:text-brand-400 mt-1 font-medium">
              Action requise
            </p>
      ...
      <div className="bg-white dark:bg-brand-900/30 rounded-3xl p-6 shadow-sm border border-brand-100 dark:border-brand-800 flex flex-wrap gap-6 items-end">
        <div className="flex flex-col gap-2.5">
          <label
            ...
            className="text-[10px] font-bold text-brand-400 dark:text-brand-500 uppercase tracking-widest"
          >
            Période
          </label>
          <div className="flex items-center gap-3">
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 bg-brand-900 dark:bg-white text-white dark:text-brand-900 rounded-2xl p-4 shadow-2xl flex items-center gap-6 animate-slide-up border border-white/10 dark:border-brand-200">
          ...
          <div className="flex gap-2">
            {activeTab !== 'quote' && (
              <button
                onClick={() => handleBulkStatusChange(InvoiceStatus.PAID)}
                className="px-4 py-2 bg-accent-500 hover:bg-accent-600 text-white rounded-xl text-xs font-bold transition-all shadow-lg shadow-accent-500/20"
            ...
            {activeTab === 'quote' && (
              <>
                <button
                  onClick={() => handleBulkStatusChange(InvoiceStatus.ACCEPTED)}
                  className="px-4 py-2 bg-accent-500 hover:bg-accent-600 text-white rounded-xl text-xs font-bold transition-all shadow-lg shadow-accent-500/20"
                  title="Accepter les devis"
                >
                  Accepter
                </button>
                <button
                  onClick={() => handleBulkStatusChange(InvoiceStatus.REJECTED)}
            ...
            <button
              onClick={() => handleBulkStatusChange(InvoiceStatus.SENT)}
              className="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-xs font-bold transition-all shadow-lg shadow-amber-500/20"
            ...
            <button
              ...
              className="p-2 hover:bg-white/10 dark:hover:bg-brand-100 rounded-xl transition-colors"
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="bg-brand-50/50 dark:bg-brand-800/50 text-brand-500 dark:text-brand-400 border-b border-brand-100 dark:border-brand-800">
                <th className="px-6 py-5 w-12 text-center">
                  ...
                  >
                    ...
                    selectedIds.size === filteredAndSortedDocuments.length ? (
                      <CheckSquare size={20} className="text-brand-900 dark:text-white" />
                    ) : (
                      <Square size={20} />
                    )}
                  </button>
                ...
                <th
                  className="px-4 py-5 font-bold uppercase tracking-widest text-[10px] cursor-pointer hover:text-brand-900 dark:hover:text-white transition-colors"
                ...
                <th
                  className="px-6 py-5 font-bold uppercase tracking-widest text-[10px] cursor-pointer hover:text-brand-900 dark:hover:text-white transition-colors"
                ...
                <th
                  className="px-6 py-5 font-bold uppercase tracking-widest text-[10px] text-right cursor-pointer hover:text-brand-900 dark:hover:text-white transition-colors"
                ...
                <th className="px-6 py-5 font-bold uppercase tracking-widest text-[10px] text-center">
                  Statut
                </th>
                <th className="px-6 py-5 font-bold uppercase tracking-widest text-[10px] text-right">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-brand-100 dark:divide-brand-800">
              {filteredAndSortedDocuments.map((doc) => {
                ...
                return (
                  <tr
                    key={doc.id}
                    className={`hover:bg-brand-50/50 dark:hover:bg-brand-800/30 transition-all group ${selectedIds.has(doc.id) ? 'bg-brand-50/80 dark:bg-brand-800/50' : ''}`}
                  >
                    <td className="px-6 py-5 text-center">
                      <button
                        onClick={() => toggleSelection(doc.id)}
                        className={`transition-all duration-300 ${selectedIds.has(doc.id) ? 'text-brand-900 dark:text-white scale-110' : 'text-brand-300 dark:text-brand-700 hover:text-brand-500'}`}
                      >
                    ...
                    <td className="px-4 py-5">
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-10 h-10 rounded-xl flex items-center justify-center ${iconBg} ${iconColor} shadow-sm transition-transform group-hover:scale-110`}
                        >
                          <Icon size={20} />
                        </div>
                        <div className="flex flex-col">
                          <span className="font-mono font-bold text-brand-900 dark:text-white">
                            {doc.number}
                          </span>
                          <div className="flex gap-2 mt-0.5">
                            {doc.linkedDocumentId && (
                              <span className="text-[10px] font-bold text-brand-400 dark:text-brand-500 flex items-center gap-1 uppercase tracking-wider">
                                <LinkIcon size={10} /> Lié
                              </span>
                            )}
                            {doc.reminderDate && (
                              <span className="text-[10px] font-bold text-orange-500 flex items-center gap-1 uppercase tracking-wider">
                                <Bell size={10} /> {new Date(doc.reminderDate).toLocaleDateString()}
                              </span>
                            )}
                          </div>
                        </div>
                    ...
                    <td className="px-6 py-5">
                      <span className="text-brand-900 dark:text-white font-bold block">
                        {clientName}
                      </span>
                      <span className="text-[10px] text-brand-400 dark:text-brand-500 uppercase font-bold tracking-widest">
                        Client ID: {doc.clientId.slice(0, 8)}
                      </span>
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5 text-right">
                      <span className="text-brand-900 dark:text-white font-black text-base">
                        {doc.total.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
                      </span>
                    </td>
                    <td className="px-6 py-5 text-center">
                      <span
                ...
                <tr>
                  <td colSpan={8} className="py-24 text-center">
                    <div className="flex flex-col items-center justify-center text-brand-300 dark:text-brand-700">
                      ...
                      <p className="text-lg font-bold text-brand-500 dark:text-brand-400 font-display">
                        Aucun document trouvé
                      </p>
                      <p className="text-sm">Modifiez vos filtres ou créez un nouveau document.</p>
                      <button onClick={startCreate} className="mt-8 btn-primary px-6 py-3">
                        <Plus size={20} />
                        Créer un document
                      </button>
                    </div>
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden animate-slide-up">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-brand-500 font-medium">
                    Décrivez ce que vous voulez facturer
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-8 space-y-6">
              <div className="bg-brand-50 p-4 rounded-2xl border border-brand-100 italic text-sm text-brand-600">
                &quot;Exemple : 3 jours de consulting IT à 500€/jour et 2 licences logicielles à
                150€ l&apos;unité.&quot;
              </div>
              <textarea
                rows={4}
