...
describe('ProductManager Component', () => {
  ...
  describe('Rendering & UI', () => {
    it('rend le gestionnaire de produits', () => {
      ...
      expect(screen.queryByText(/Produits|Products/i) || screen.queryByText(/Package/i)).toBeDefined();
    });
  ...
  describe('Category Filtering', () => {
    ...
    it('filtre par catégorie Services', async () => {
      ...
      const filterButtons = screen.queryAllByRole('button').filter(b =>
    ...
    it('filtre par catégorie Fournitures', async () => {
      ...
      const filterButtons = screen.queryAllByRole('button').filter(b =>
        b.textContent?.includes('Fournitures')
      );
  ...
  describe('Sorting', () => {
    ...
    it('trie par prix', async () => {
      ...
      const sortButtons = screen.queryAllByRole('button').filter(b =>
        b.textContent?.includes('Prix') || b.textContent?.toLowerCase().includes('price')
      );
    ...
    it('trie par catégorie', async () => {
      ...
      const sortButtons = screen.queryAllByRole('button').filter(b =>
        b.textContent?.includes('Catégorie') || b.textContent?.toLowerCase().includes('category')
      );
  ...
  describe('Stock Management', () => {
    ...
    it('filtre pour afficher uniquement les produits à stock faible', async () => {
      ...
      const lowStockButtons = screen.queryAllByRole('button').filter(b =>
        b.textContent?.includes('Faible') || b.textContent?.includes('Low') || b.textContent?.includes('stock')
      );
  ...
  describe('Add Product Workflow', () => {
    it('ouvre le panneau pour créer un produit', async () => {
      ...
      const addButton = screen.getAllByRole('button').find(b => b.textContent?.includes('Nouveau'));
    ...
    it('valide que le nom est requis', async () => {
      ...
      const submitButton = screen.queryAllByRole('button').find(b =>
        b.textContent?.includes('Créer') || b.textContent?.includes('Ajouter')
      );
  ...
  describe('Archive Product', () => {
    ...
    it('affiche les produits archivés si filtré', async () => {
      ...
      const showArchivedButton = screen.queryAllByRole('button').find(b =>
        b.textContent?.includes('Archiv')
      );
