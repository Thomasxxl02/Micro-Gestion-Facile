...
try {
    ...
    $mainBundle = Get-ChildItem "dist/assets/index-*.js" | Select-Object -First 1
    if ($null -ne $mainBundle) {
        $bundleSize = $mainBundle.Length
        Print-Success "Production build completed (bundle size: $bundleSize bytes)"
    } else {
        Print-Warning "Production build completed, but main bundle (index-*.js) not found for size check."
    }
} catch {
