...
const ProductManager: React.FC<ProductManagerProps> = ({ products, setProducts, onSave, onDelete }) => {
  ...
  const exportCSV = () => {
    const headers = ['Référence', 'Nom', 'Type', 'Catégorie', 'Prix', 'Unité', 'Stock', 'Seuil Alerte', 'Description', 'Statut'];
  ...
  const processedProducts = useMemo(() => {
    ...
    if (showLowStockOnly) {
        result = result.filter(p => p.type === 'product' && p.stock !== undefined && p.stock <= (p.minStock || 0));
    }
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        ...
        <div className="flex flex-wrap gap-3 items-center w-full lg:w-auto">
            ...
            <div className="flex gap-2">
                <label className="btn-secondary px-4 py-2.5 cursor-pointer" title="Importer depuis un CSV">
                    <Upload size={18} />
      ...
      <div className="bento-grid">
          <div className="bento-item">
              <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Total Catalogue</span>
              </div>
              <div>
                  <h3 className="text-2xl font-bold text-[var(--text-main)] font-display">{globalStats.total}</h3>
                  <p className="text-xs text-[var(--text-muted)] mt-1">Éléments actifs</p>
              </div>
          </div>
          <div className="bento-item">
              <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Prestations</span>
              </div>
              <div>
                  <h3 className="text-2xl font-bold text-accent-600 dark:text-accent-400 font-display">{globalStats.servicesCount}</h3>
                  <p className="text-xs text-[var(--text-muted)] mt-1">Services proposés</p>
              </div>
          </div>
          <div className="bento-item">
              <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Stock Bas</span>
              </div>
              <div>
                  <h3 className="text-2xl font-bold text-amber-600 dark:text-amber-400 font-display">{globalStats.lowStockCount}</h3>
                  <p className="text-xs text-[var(--text-muted)] mt-1">Articles à réapprovisionner</p>
              </div>
          </div>
       ...
       {/* Side Panel Form */}
       <div className={`fixed inset-y-0 right-0 w-full sm:w-[450px] bg-[var(--card-bg)] shadow-2xl transform transition-transform duration-300 ease-in-out z-40 border-l border-[var(--card-border)] ${isPanelOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-full flex flex-col">
            <div className="p-6 border-b border-[var(--card-border)] flex justify-between items-center bg-[var(--bg-main)]/50">
                <h3 className="text-xl font-bold text-[var(--text-main)]">{editingId ? 'Modifier l\'élément' : 'Nouvel élément'}</h3>
                <button onClick={() => setIsPanelOpen(false)} className="p-2 hover:bg-[var(--bg-main)] rounded-full text-[var(--text-muted)] transition-colors">
                    <X size={20} />
                </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-6">
                <div className="space-y-6">
                    <div>
                        <label className="block text-sm font-semibold text-[var(--text-main)] mb-1.5">Nom de l'élément <span className="text-red-500">*</span></label>
                        <input 
                            type="text" 
                            required
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {processedProducts.map(p => (
                ...
                >
                    ...
                    <div className="mb-6">
                        ...
                        <h4 className="font-bold text-brand-900 dark:text-white text-xl leading-tight mb-3 pr-10 font-display">{p.name}</h4>
                        <p className="text-sm text-brand-500 dark:text-brand-400 line-clamp-2 h-10 leading-relaxed">{p.description || "Aucune description"}</p>
                    </div>

                    <div className="mt-auto pt-6 border-t border-brand-50 dark:border-brand-800 flex items-center justify-between">
                        <div className="flex flex-col">
                            <span className="text-[9px] uppercase font-black text-brand-400 dark:text-brand-500 tracking-widest mb-1">Prix HT</span>
                            <div className="flex items-baseline gap-1.5">
                                <span className="text-2xl font-bold text-brand-900 dark:text-white font-display">{p.price.toFixed(2)} €</span>
                                <span className="text-[10px] text-brand-400 dark:text-brand-500 font-bold uppercase tracking-widest">/ {p.unit || 'unité'}</span>
                            </div>
                        </div>
                        ...
                        {p.type === 'product' && p.stock !== undefined && (
                            <div className="text-right">
                                ...
                                <div className="flex items-center gap-2 bg-brand-50 dark:bg-brand-800 p-1.5 rounded-xl border border-brand-100 dark:border-brand-700">
                                    ...
                                    <div className={`flex items-center gap-1.5 font-black px-2 ${p.stock <= (p.minStock || 0) ? 'text-red-500 dark:text-red-400' : 'text-brand-900 dark:text-white'}`}>
            ...
            {processedProducts.length === 0 && (
                 <div className="col-span-full py-32 text-center">
                    ...
                    <h3 className="text-brand-900 dark:text-white font-bold text-xl font-display mb-2">Catalogue vide</h3>
                    <p className="text-brand-400 dark:text-brand-500 text-sm max-w-xs mx-auto">Essayez une autre recherche ou changez de filtre pour trouver ce que vous cherchez.</p>
                </div>
            )}
        </div>
      </div>
