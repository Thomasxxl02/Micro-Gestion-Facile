...
const Dashboard: React.FC<DashboardProps> = ({ invoices, products, expenses, emails = [], events = [], onNavigate, userProfile }) => {
  ...
  const quickActions = [
    ...
    { label: 'Dépense', icon: TrendingDown, onClick: () => onNavigate('accounting') },
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto pb-10">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
        <div>
           <h2 className="text-3xl font-bold tracking-tight font-display rounded-lg px-2 py-1 -ml-2 w-fit" style={{ backgroundColor: '#ffffff', color: '#498ed3' }}>Tableau de bord</h2>
                ...
                <React.Fragment key={id}>
                  ...
                  <SortableWidget id="stats-revenue" className="lg:col-span-1">
                    <div className="card-modern p-6 h-full flex flex-col justify-between relative overflow-hidden group bg-pastel-blue dark:bg-brand-900 border-l-4 border-l-vibrant-blue hover:border-l-blue-600 transition-all duration-500">
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-blue-800/70 dark:text-brand-500 uppercase tracking-[0.2em] mb-1">Chiffre d'Affaires</p>
                        <div className="flex items-baseline gap-2">
                          <h3 className="text-3xl font-black text-blue-950 dark:text-white tracking-tighter font-display">
                            {totalRevenue.toLocaleString('fr-FR')} €
                          </h3>
                          <div className={`flex items-center text-[10px] font-bold px-2 py-0.5 rounded-lg ${cashFlowStats.percent >= 0 ? 'text-emerald-700 bg-emerald-100/80 dark:bg-emerald-900/20' : 'text-rose-700 bg-rose-100/80 dark:bg-rose-900/20'}`}>
                  ...
                  <SortableWidget id="stats-profit" className="lg:col-span-1">
                    <div className="bg-linear-to-br from-vibrant-indigo to-vibrant-purple dark:from-brand-950 dark:to-black text-white p-6 rounded-[2.5rem] h-full shadow-2xl shadow-vibrant-indigo/30 dark:shadow-brand-900/20 relative overflow-hidden group border border-white/10 transition-all duration-500 hover:-translate-y-1">
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-white/80 uppercase tracking-[0.2em] mb-1">Résultat Brut</p>
                    ...
                    <div className="card-modern p-6 h-full flex flex-col justify-between border-l-4 border-l-vibrant-emerald bg-pastel-green dark:bg-brand-900 hover:border-l-emerald-600 transition-all duration-500">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="group relative">
                            <AlertCircle size={14} className="text-emerald-600 dark:text-emerald-300 cursor-help" />
                      ...
                      <div>
                        <p className="text-[10px] font-bold text-emerald-800/70 dark:text-brand-500 uppercase tracking-[0.2em] mb-1">Revenu Net Est.</p>
                  ...
                  <SortableWidget id="stats-vat" className="lg:col-span-1">
                    <div className="card-modern p-6 h-full flex flex-col justify-between relative overflow-hidden bg-pastel-yellow dark:bg-brand-900 border-l-4 border-l-vibrant-amber hover:border-l-amber-600 transition-all duration-500">
                      <div className="relative z-10">
                         <p className="text-[10px] font-bold text-amber-800/70 dark:text-brand-500 uppercase tracking-[0.2em] mb-1">Franchise TVA</p>
                         <div className="flex justify-between items-end mb-3">
                            <h3 className="text-3xl font-black text-amber-950 dark:text-amber-400 tracking-tighter font-display">{vatProgress.toFixed(0)}%</h3>
                         ...
                         {vatProgress > 85 && (
                           <p className="text-[8px] text-rose-600 dark:text-rose-500 font-bold mt-2 animate-pulse uppercase tracking-widest">Seuil de TVA imminent !</p>
                         )}
                      </div>
                    </div>
                  </SortableWidget>
                    ...
                    <div className="flex-1 min-h-[300px] w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={monthlyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                          ...
                          />
                          <Area type="monotone" dataKey="Recettes" stroke="var(--brand-accent)" strokeWidth={4} fillOpacity={1} fill="url(#colorRecettes)" />
                          <Area type="monotone" dataKey="Dépenses" stroke="#f43f5e" strokeWidth={4} fillOpacity={1} fill="url(#colorDepenses)" />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                    ...
                    <div className="flex-1 overflow-auto -mx-2 px-2 custom-scrollbar">
                      <div className="space-y-3">
                        {recentActivity.map((item) => (
                          <div key={item.id} className="flex items-center justify-between p-4 hover:bg-brand-50 dark:hover:bg-brand-800/50 rounded-3xl transition-all cursor-pointer border border-transparent hover:border-brand-100 dark:hover:border-brand-800 group shadow-sm hover:shadow-md">
                            <div className="flex items-center gap-4">
                              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xs font-bold shadow-inner
                                ${item.activityType === 'invoice' ? (
                                  (item as Invoice).status === InvoiceStatus.PAID ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400' : 
                                ...
                                {item.activityType === 'invoice' ? <FileText size={20} /> : <ShoppingCart size={20} />}
                              </div>
                              <div>
                                ...
                                <p className="text-[10px] font-bold text-brand-400 uppercase tracking-widest mt-0.5">{new Date(item.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}</p>
                              </div>
                            </div>
                            <div className="text-right">
                              <p className={`text-sm font-black ${item.activityType === 'expense' || (item as Invoice).type === 'credit_note' ? 'text-rose-500' : 'text-brand-900 dark:text-brand-50'}`}>
                              ...
                              <span className={`text-[9px] font-black px-2.5 py-1 rounded-lg mt-2 inline-block uppercase tracking-widest
                                   ${item.activityType === 'invoice' ? (
                                     (item as Invoice).status === InvoiceStatus.PAID ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400' : 
