            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-brand-900 text-white">
                  <th className="px-6 py-4 text-xs font-bold uppercase tracking-wider">Destinataire</th>
                  ...
                  return (
                    <tr key={email.id} className="hover:bg-brand-50/50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="font-bold text-brand-900">{client ? client.name : email.to}</div>
                        <div className="text-[10px] text-brand-400 uppercase font-bold flex items-center gap-2">
                          {email.type}
                          {client && <span className="text-brand-300 font-normal lowercase italic">({email.to})</span>}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-brand-600 truncate max-w-xs">{email.subject}</td>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${
                        email.status === 'sent' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'
                      }`}>
                        {email.status === 'sent' ? <CheckCircle2 size={12} /> : <AlertCircle size={12} />}
      ...
      {activeTab === 'templates' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {templates.map(template => (
            <div key={template.id} className="bg-white border border-brand-100 rounded-[2rem] p-6 shadow-sm hover:shadow-md transition-all group relative">
              ...
              <div className="space-y-6 animate-fade-in">
                <div className="p-6 bg-brand-50 rounded-2xl border border-brand-100">
                  <div className="mb-4 pb-4 border-b border-brand-200">
                    ...
                    <div className="text-brand-900 font-medium">{composeData.to || '(Non spécifié)'}</div>
                  </div>
                  <div className="mb-4 pb-4 border-b border-brand-200">
                    <div className="text-xs font-bold text-brand-400 uppercase mb-1">Sujet:</div>
                    <div className="text-brand-900 font-bold">{composeData.subject || '(Sans objet)'}</div>
                  </div>
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        {invoices
                          ...
                          .map(i => (
                            <option key={i.id} value={i.id}>{i.number} - {i.total.toFixed(2)}€</option>
                          ))
                        }
                      </select>
                    </div>
            ...
            <div className="bg-brand-50 border border-brand-100 rounded-[2rem] p-6">
              ...
              <div className="space-y-3">
                {templates.map(t => (
                  ...
                  >
                    <div className="text-xs font-bold text-brand-900 group-hover:text-brand-600">{t.name}</div>
                    <div className="text-[10px] text-brand-400 uppercase font-bold mt-1">{t.type}</div>
                  </button>
                ))}
                {templates.length === 0 && (
