...
describe('useEntityStats Hook', () => {
  ...
  describe('Statistics Calculation', () => {
    it('calcule le total des factures', () => {
      const { result } = renderHook(() =>
        useEntityStats(mockInvoices, {
          totalRevenue: (items: Invoice[]) =>
            items.reduce((sum, inv) => sum + inv.total, 0),
        })
      );
    ...
    it('compte les factures payées', () => {
      const { result } = renderHook(() =>
        useEntityStats(mockInvoices, {
          paidCount: (items: Invoice[]) =>
            items.filter((inv) => inv.status === 'paid').length,
        })
      );
    ...
    it('calcule plusieurs statistiques simultanément', () => {
      const { result } = renderHook(() =>
        useEntityStats(mockInvoices, {
          ...
          paidCount: (items: Invoice[]) =>
            items.filter((inv) => inv.status === 'paid').length,
          draftCount: (items: Invoice[]) =>
            items.filter((inv) => inv.status === 'draft').length,
        })
      );
  ...
  describe('Memoization', () => {
    it('recalcule les stats si les données changent', () => {
      ...
      const { result, rerender } = renderHook(
        ({ items }) => useEntityStats(items, stats),
        { initialProps: { items: mockInvoices } }
      );

      expect(result.current.count).toBe(3);

      const newInvoices: Invoice[] = [...mockInvoices, {
        id: 'inv-4',
        number: 'FAC-004',
        date: '2026-03-01',
        dueDate: '2026-06-01',
      ...
      const { result } = renderHook(() =>
        useEntityStats(mockInvoices, {
          avgAmount: (items: Invoice[]) =>
            items.length > 0
              ? items.reduce((sum, inv) => sum + inv.total, 0) / items.length
              : 0,
        })
      );
