...
import {
  Key, Smartphone, KeyRound, LogOut, Eye, EyeOff,
  AlertTriangle, CheckCircle, Copy, Trash2, Plus, Clock, MapPin,
  Lock, Zap, History
} from 'lucide-react';
import {
  TOTPService, APIKeyService, SessionService, PasswordResetService,
  DataEncryptionService, PasswordValidator,
  type APIKey, type Session, type LoginHistoryEntry
...
const SecurityTab: React.FC<SecurityTabProps> = ({
  ...
  const [newAPIKeyService, setNewAPIKeyService] = useState<'GEMINI' | 'FIREBASE' | 'CUSTOM'>('GEMINI');
  ...
  // ─── DIALOGS ───
  const [confirmDialog, setConfirmDialog] = useState<{ isOpen: boolean; title: string; description: string; isDangerous?: boolean; onConfirm?: () => void }>({ isOpen: false, title: '', description: '' });
  const [alertDialog, setAlertDialog] = useState<{ isOpen: boolean; title: string; description: string; type: 'success' | 'error' | 'info' }>({ isOpen: false, title: '', description: '', type: 'info' });
  ...
  const handleDisable2FA = () => {
    setConfirmDialog({
      ...
      description: 'Vous perdrez la protection supplémentaire. Cette action ne peut être annulée que par reset du profil.',
      isDangerous: true,
      onConfirm: () => {
        const updatedProfile = {
          ...userProfile,
  ...
  const handleCreateAPIKey = async () => {
    ...
    const newKey = await APIKeyService.createAPIKey(newAPIKeyName, newAPIKeyService, newAPIKeyValue);
  ...
  const handleRevokeAPIKey = (keyId: string) => {
    setConfirmDialog({
      ...
      description: 'Cette action désactivera la clé imédiatement. Vous devrez en créer une nouvelle.',
      isDangerous: true,
      onConfirm: () => {
    ...
    >
      ...
      <div className="flex flex-wrap gap-2 bg-brand-50 dark:bg-brand-900/30 p-3 rounded-2xl border border-brand-100 dark:border-brand-800">
        {['2FA', 'API Keys', 'Mot de passe', 'Sessions', 'Chiffrement'].map((label, idx) => {
          const tabKeys: Array<'2fa' | 'api-keys' | 'password' | 'sessions' | 'encryption'> = ['2fa', 'api-keys', 'password', 'sessions', 'encryption'];
                ...
                <div className="space-y-6">
                  <div className="bg-brand-50 dark:bg-brand-800/50 p-6 rounded-2xl border-2 border-dashed border-brand-300 dark:border-brand-700 text-center">
                    ...
                    <div className="mt-6 p-4 bg-white dark:bg-brand-900 rounded-xl border border-brand-200 dark:border-brand-700">
                      <p className="text-xs text-brand-500 dark:text-brand-400 mb-2">Ou entrez ce code manuellement:</p>
        ...
        <div className="bg-white dark:bg-brand-900/50 rounded-4xl p-8 shadow-sm border border-brand-100 dark:border-brand-800">
          ...
          <div className="space-y-6">
            <div className="p-4 bg-brand-50 dark:bg-brand-800/50 rounded-2xl border border-brand-200 dark:border-brand-700">
              <label htmlFor="confirmationEmail" className="text-sm font-semibold text-brand-900 dark:text-white block mb-2">
              ...
              />
              <p className="text-xs text-brand-500 dark:text-brand-400 mt-2">Email du compte (lecture seule)</p>
            </div>
          ...
          {loginHistory.length > 0 && (
            <div className="bg-white dark:bg-brand-900/50 rounded-4xl p-8 shadow-sm border border-brand-100 dark:border-brand-800">
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto">
                {loginHistory.slice(-10).reverse().map((entry) => (
                  <div
                    key={entry.id}
            ...
            <div className="space-y-6">
              <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl flex items-start gap-3">
                ...
                <p className="text-sm text-amber-700 dark:text-amber-300">
                  Vos données IBAN, SIRET et autres informations sensibles ne sont pas encore chiffrées.
                  Activez le chiffrement pour une sécurité maximale.
                </p>
              </div>
              ...
              <div className="bg-brand-50 dark:bg-brand-800/50 p-4 rounded-2xl border border-brand-200 dark:border-brand-700">
                <p className="text-xs text-brand-600 dark:text-brand-400 mb-3 font-semibold">ℹ️ Informations de sécurité:</p>
