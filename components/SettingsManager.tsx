...
import { UserProfile, Invoice, Client, Supplier, Product, Expense, InvoiceStatus, Theme } from '../types';
import { 
  ...
  Sun, Moon, Monitor, Bell, Calendar, Clock, Save as SaveIcon,
  FileJson, Table, Send, PenTool
} from 'lucide-react';
...
const SettingsManager: React.FC<SettingsManagerProps> = ({ 
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'billing' | 'data' | 'preferences' | 'integrations' | 'communications'>('profile');
  ...
  const [siretStatus, setSiretStatus] = useState<'idle' | 'valid' | 'invalid'>('idle');
  const checkSiret = () => {
  ...
  const handleExportCSV = async () => {
    ...
    if (allData.invoices.length) zip.file("factures.csv", generateCSV(allData.invoices));
    if (allData.clients.length) zip.file("clients.csv", generateCSV(allData.clients));
    if (allData.products.length) zip.file("produits.csv", generateCSV(allData.products));
    if (allData.expenses.length) zip.file("depenses.csv", generateCSV(allData.expenses));
    if (allData.suppliers.length) zip.file("fournisseurs.csv", generateCSV(allData.suppliers));
  ...
  const parseSQL = (sqlText: string) => {
    const data: any = { invoices: [], clients: [], products: [], expenses: [], suppliers: [] };
  ...
  const handleImportAll = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (file.name.endsWith('.zip')) {
        ...
        if (file.name.endsWith('.json')) {
          ...
          if (parsed.length > 0) {
            ...
            else if (headers.includes('category') && headers.includes('address')) data.clients = parsed;
            else if (headers.includes('price') && headers.includes('unit')) data.products = parsed;
            else if (headers.includes('amount') && headers.includes('date')) data.expenses = parsed;
      ...
      if (data.invoices && data.invoices.length) setAllData.setInvoices(data.invoices);
      if (data.clients && data.clients.length) setAllData.setClients(data.clients);
      if (data.suppliers && data.suppliers.length) setAllData.setSuppliers(data.suppliers);
      if (data.products && data.products.length) setAllData.setProducts(data.products);
      if (data.expenses && data.expenses.length) setAllData.setExpenses(data.expenses);
      
      alert('Données importées avec succès !');
    } catch (err) {
      console.error(err);
      alert('Erreur lors de l\'importation. Fichier invalide ou format non reconnu.');
    }
  ...
  return (
    <div className="max-w-7xl mx-auto animate-fade-in pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
        <div>
          <h2 className="text-3xl font-black text-brand-900 dark:text-white font-display tracking-tighter">Paramètres</h2>
          <p className="text-brand-500 dark:text-brand-400 mt-1 text-sm font-medium">Gérez votre profil, votre image de marque et vos données.</p>
        </div>
              ...
              <div className="space-y-8 animate-slide-up">
                ...
                <div className="bg-pastel-blue/30 dark:bg-brand-900 rounded-[2.5rem] p-8 shadow-xl shadow-brand-900/5 border border-white/50 dark:border-brand-800 backdrop-blur-sm">
                    <div className="flex items-center gap-3 mb-8 border-b border-brand-100/50 dark:border-brand-800 pb-6">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-brand-900 dark:text-white font-display tracking-tight">Identité Professionnelle</h3>
                            <p className="text-[10px] text-brand-500 font-bold uppercase tracking-widest mt-0.5">Informations légales de votre structure</p>
                        </div>
                    </div>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="md:col-span-2">
                            <label className="block text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-3">Nom commercial / Raison sociale</label>
                            <input 
                                type="text" 
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-3">Titre Professionnel</label>
                            <div className="relative group">
                                <Briefcase className="absolute left-5 top-1/2 -translate-y-1/2 text-brand-300 dark:text-brand-600 group-focus-within:text-blue-500 transition-colors" size={18} />
                                <input 
                                    type="text" 
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-3">Numéro SIRET</label>
                            <input 
                                type="text" 
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-3">Numéro SIREN</label>
                            <input 
                                type="text" 
                                className="w-full p-5 bg-brand-50/50 dark:bg-brand-800/30 border border-brand-100 dark:border-brand-800 rounded-2xl outline-none focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 transition-all font-mono text-sm font-black text-brand-900 dark:text-white" 
                ...
                <div className="bg-pastel-pink/30 dark:bg-brand-900 rounded-[2.5rem] p-8 shadow-xl shadow-brand-900/5 border border-white/50 dark:border-brand-800 backdrop-blur-sm">
                    <div className="flex items-center gap-3 mb-8 border-b border-brand-100/50 dark:border-brand-800 pb-6">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-brand-900 dark:text-white font-display tracking-tight">Coordonnées & Web</h3>
                            <p className="text-[10px] text-brand-500 font-bold uppercase tracking-widest mt-0.5">Vos points de contact</p>
                        </div>
                    </div>
              ...
              <div className="space-y-8 animate-slide-up">
                ...
                <div className="bg-pastel-purple/30 dark:bg-brand-900 rounded-[2.5rem] p-8 shadow-xl shadow-brand-900/5 border border-white/50 dark:border-brand-800 backdrop-blur-sm">
                    <div className="flex items-center gap-3 mb-8 border-b border-brand-100/50 dark:border-brand-800 pb-6">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-brand-900 dark:text-white font-display tracking-tight">Identité Visuelle</h3>
                            <p className="text-[10px] text-brand-500 font-bold uppercase tracking-widest mt-0.5">Personnalisation de vos documents</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div>
                                <label className="block text-[10px] font-bold text-brand-400 uppercase tracking-widest mb-2">Logo (URL ou Base64)</label>
                                <div className="relative">
                                    <input 
                                        type="text" 
                                    ...
                                    {userProfile.logoUrl && (
                                        <div className="mt-4 p-4 bg-white rounded-2xl border border-brand-100 flex items-center justify-center h-24">
                                            <img src={userProfile.logoUrl} alt="Logo preview" className="max-h-full max-w-full object-contain" />
                                        </div>
                                    )}
                                </div>
                            </div>
                ...
                <div className="bg-pastel-green/30 dark:bg-brand-900 rounded-[2.5rem] p-8 shadow-xl shadow-brand-900/5 border border-white/50 dark:border-brand-800 backdrop-blur-sm">
                    <div className="flex items-center gap-3 mb-8 border-b border-brand-100/50 dark:border-brand-800 pb-6">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-brand-900 dark:text-white font-display tracking-tight">Bancaire & Légal</h3>
                            <p className="text-[10px] text-brand-500 font-bold uppercase tracking-widest mt-0.5">Informations de facturation</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                            ...
                            <div>
                                <label className="block text-[10px] font-bold text-brand-400 uppercase tracking-widest mb-2">Cotisations URSSAF (%)</label>
                                <input 
                                    type="number" 
                                    step="0.1"
                                    className="w-full p-4 bg-brand-50/50 border border-brand-100 rounded-2xl outline-none focus:ring-4 focus:ring-brand-900/5 focus:border-brand-900 transition-all font-bold text-brand-900" 
                                    value={userProfile.socialContributionRate || 21.1}
                                    onChange={(e) => handleChange('socialContributionRate', parseFloat(e.target.value))}
                                    placeholder="Ex: 21.1"
                                />
                            </div>
                        </div>
              ...
              <div className="space-y-8 animate-slide-up">
                <div className="bg-pastel-yellow/30 dark:bg-brand-900 rounded-[2.5rem] p-8 shadow-xl shadow-brand-900/5 border border-white/50 dark:border-brand-800 backdrop-blur-sm">
                    <div className="flex items-center gap-3 mb-8 border-b border-brand-100/50 dark:border-brand-800 pb-6">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-brand-900 dark:text-white font-display tracking-tight">Interface & Préférences</h3>
                            <p className="text-[10px] text-brand-500 font-bold uppercase tracking-widest mt-0.5">Personnalisez votre expérience</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            <label className="block text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-3">Langue de l'interface</label>
                            <div className="relative group">
                                <Globe className="absolute left-5 top-1/2 -translate-y-1/2 text-brand-300 dark:text-brand-600 group-focus-within:text-blue-500 transition-colors" size={18} />
                                <select 
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-3">Thème de l'application</label>
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-3">Format de date</label>
                            <div className="relative group">
                                <Calendar className="absolute left-5 top-1/2 -translate-y-1/2 text-brand-300 dark:text-brand-600 group-focus-within:text-blue-500 transition-colors" size={18} />
                                <select 
                                    className="w-full pl-14 p-5 bg-brand-50/50 dark:bg-brand-800/30 border border-brand-100 dark:border-brand-800 rounded-2xl font-black text-brand-900 dark:text-white outline-none appearance-none cursor-pointer focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 transition-all"
                                    value={userProfile.preferences?.dateFormat || 'DD/MM/YYYY'}
                        ...
                        <div>
                            <label className="block text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-3">Délai de paiement par défaut (jours)</label>
                            <div className="relative group">
                                <Clock className="absolute left-5 top-1/2 -translate-y-1/2 text-brand-300 dark:text-brand-600 group-focus-within:text-blue-500 transition-colors" size={18} />
                                <input 
                                    type="number"
                                    ...
                                    value={userProfile.preferences?.defaultDueDateDays || 30}
                                    onChange={(e) => handlePreferenceChange('defaultDueDateDays', parseInt(e.target.value))}
                                />
                            </div>
                        </div>

                        <div className="md:col-span-2 pt-8 border-t border-brand-50 dark:border-brand-800">
                            <h4 className="text-sm font-black text-brand-900 dark:text-white uppercase tracking-widest mb-6">Affichage des Devises & Arrondis</h4>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    <label className="block text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-3">Symbole Monétaire</label>
                                    <input 
                                        type="text"
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-3">Position du symbole</label>
                                    <select 
                                        ...
                                        value={userProfile.preferences?.currencyPosition || 'after'}
                                ...
                                <div>
                                    <label className="block text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-3">Mode d'arrondi</label>
                                    <select 
                                        className="w-full p-5 bg-brand-50/50 dark:bg-brand-800/30 border border-brand-100 dark:border-brand-800 rounded-2xl font-black text-brand-900 dark:text-white outline-none appearance-none cursor-pointer focus:ring-4 focus:ring-blue-500/5 focus:border-blue-500 transition-all"
                                        value={userProfile.preferences?.roundingMode || 'nearest'}
                                        onChange={(e) => handlePreferenceChange('roundingMode', e.target.value)}
                                    >
                                        <option value="nearest">Au plus proche (Standard)</option>
                        ...
                        <div className="md:col-span-2 pt-8 border-t border-brand-50 dark:border-brand-800">
                            <h4 className="text-sm font-black text-brand-900 dark:text-white uppercase tracking-widest mb-6">Outils de Vérification</h4>
                            <div className="p-8 bg-brand-50 dark:bg-brand-800/30 rounded-[2.5rem] border border-brand-100 dark:border-brand-800 shadow-inner">
                                <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                                    <div>
                                        <p className="text-lg font-black text-brand-900 dark:text-white tracking-tight">Vérificateur de SIRET</p>
                                        <p className="text-xs text-brand-400 font-medium mt-1">Valider la conformité de votre numéro SIRET (Algorithme de Luhn)</p>
                                    </div>
                                    <div className="flex items-center gap-6">
                                        ...
                                        >
                                            {siretStatus === 'valid' ? 'Valide !' : siretStatus === 'invalid' ? 'Invalide' : 'Vérifier'}
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="md:col-span-2 pt-8 border-t border-brand-50 dark:border-brand-800">
                            <h4 className="text-sm font-black text-brand-900 dark:text-white uppercase tracking-widest mb-6">Automatisation & Notifications</h4>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="flex items-center justify-between p-6 bg-brand-50/50 dark:bg-brand-800/50 rounded-[2rem] border border-brand-100 dark:border-brand-800 hover:border-brand-200 dark:hover:border-brand-700 transition-all group">
                                    <div className="flex gap-5 items-center">
                                        ...
                                        <div>
                                            <p className="text-base font-black text-brand-900 dark:text-white tracking-tight">Sauvegarde automatique</p>
                                            <p className="text-xs text-brand-400 font-medium">Enregistrer les modifications en temps réel</p>
                                        </div>
                                    </div>
                                    <button 
                                        onClick={() => handlePreferenceChange('autoSave', !userProfile.preferences?.autoSave)}
                                    ...
                                    >
                                        <div className={`absolute top-1 w-5 h-5 bg-white rounded-full shadow-md transition-all ${userProfile.preferences?.autoSave ? 'right-1' : 'left-1'}`}></div>
                                    </button>
                                </div>
                                ...
                                <div className="flex items-center justify-between p-6 bg-brand-50/50 dark:bg-brand-800/50 rounded-[2rem] border border-brand-100 dark:border-brand-800 hover:border-brand-200 dark:hover:border-brand-700 transition-all group">
                                    <div className="flex gap-5 items-center">
                                        ...
                                        <div>
                                            <p className="text-base font-black text-brand-900 dark:text-white tracking-tight">Notifications système</p>
                                            <p className="text-xs text-brand-400 font-medium">Activer les alertes et rappels visuels</p>
                                        </div>
                                    </div>
                                    <button 
                                        onClick={() => handlePreferenceChange('notificationsEnabled', !userProfile.preferences?.notificationsEnabled)}
                                    ...
                                    >
                                        <div className={`absolute top-1 w-5 h-5 bg-white rounded-full shadow-md transition-all ${userProfile.preferences?.notificationsEnabled ? 'right-1' : 'left-1'}`}></div>
                                    </button>
                                </div>
                            </div>
                        </div>
            ...
            {activeTab === 'data' && (
              <div className="space-y-8 animate-slide-up">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div className="bg-pastel-blue/30 dark:bg-brand-900 p-6 rounded-[2rem] border border-white/50 dark:border-brand-800 shadow-xl shadow-brand-900/5 text-center group hover:scale-105 transition-all backdrop-blur-sm">
                        <p className="text-[10px] font-black text-vibrant-blue dark:text-brand-500 uppercase tracking-widest mb-2">Factures</p>
                        <p className="text-3xl font-black text-blue-950 dark:text-white font-display tracking-tighter">{allData.invoices.length}</p>
                    </div>
                    <div className="bg-pastel-purple/30 dark:bg-brand-900 p-6 rounded-[2rem] border border-white/50 dark:border-brand-800 shadow-xl shadow-brand-900/5 text-center group hover:scale-105 transition-all backdrop-blur-sm">
                        <p className="text-[10px] font-black text-vibrant-purple dark:text-brand-500 uppercase tracking-widest mb-2">Clients</p>
                        <p className="text-3xl font-black text-purple-950 dark:text-white font-display tracking-tighter">{allData.clients.length}</p>
                    </div>
                    <div className="bg-pastel-yellow/30 dark:bg-brand-900 p-6 rounded-[2rem] border border-white/50 dark:border-brand-800 shadow-xl shadow-brand-900/5 text-center group hover:scale-105 transition-all backdrop-blur-sm">
                        <p className="text-[10px] font-black text-vibrant-amber dark:text-brand-500 uppercase tracking-widest mb-2">Produits</p>
                        <p className="text-3xl font-black text-amber-950 dark:text-white font-display tracking-tighter">{allData.products.length}</p>
                    </div>
                    <div className="bg-pastel-green/30 dark:bg-brand-900 p-6 rounded-[2rem] border border-white/50 dark:border-brand-800 shadow-xl shadow-brand-900/5 text-center group hover:scale-105 transition-all backdrop-blur-sm">
                        <p className="text-[10px] font-black text-vibrant-emerald dark:text-brand-500 uppercase tracking-widest mb-2">Dépenses</p>
                        <p className="text-3xl font-black text-emerald-950 dark:text-white font-display tracking-tighter">{allData.expenses.length}</p>
                    </div>
                </div>
                ...
                <div className="bg-pastel-orange/30 dark:bg-brand-900 p-8 rounded-[2.5rem] border border-white/50 dark:border-brand-800 shadow-xl shadow-brand-900/5 backdrop-blur-sm">
                    <div className="flex items-center gap-3 mb-8 border-b border-brand-100/50 dark:border-brand-800 pb-6">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-brand-900 dark:text-white font-display tracking-tight">Sauvegarde & Maintenance</h3>
                            <p className="text-[10px] text-brand-500 font-bold uppercase tracking-widest mt-0.5">Gérez l'intégrité de vos données</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                        <div className="space-y-3">
                            <label className="block text-[10px] font-bold text-brand-400 uppercase tracking-widest">Exporter les données</label>
                        ...
                        <div className="space-y-3">
                            <label className="block text-[10px] font-bold text-brand-400 uppercase tracking-widest">Importer des données</label>
                            <button 
                    ...
                    <div className="p-8 bg-amber-50 dark:bg-amber-900/20 rounded-[2rem] border border-amber-100 dark:border-amber-900/30 mb-8">
                        <div className="flex gap-5">
                            <div className="p-3 bg-white dark:bg-brand-900 rounded-2xl text-amber-600 dark:text-amber-500 h-fit shadow-sm">
                                <ShieldAlert size={24} />
                            </div>
                            <div>
                                <p className="text-lg font-black text-amber-900 dark:text-amber-400 tracking-tight mb-1">Sécurité des données</p>
                    ...
                    <div className="border-t border-brand-50 dark:border-brand-800 pt-8">
                        <p className="text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-6">Zone de Danger</p>
              ...
              <div className="space-y-8 animate-slide-up">
                <div className="bg-pastel-orange/30 dark:bg-brand-900 rounded-[2.5rem] p-8 shadow-xl shadow-brand-900/5 border border-white/50 dark:border-brand-800 backdrop-blur-sm">
                    <div className="flex items-center gap-3 mb-8 border-b border-brand-100/50 dark:border-brand-800 pb-6">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-brand-900 dark:text-white font-display tracking-tight">Emails & Communications</h3>
                            <p className="text-[10px] text-brand-500 font-bold uppercase tracking-widest mt-0.5">Personnalisez vos envois</p>
                        </div>
                    </div>

                    <div className="space-y-8">
                        <div>
                            <label className="block text-[10px] font-bold text-brand-400 uppercase tracking-widest mb-2">Signature d'email par défaut</label>
                            <div className="relative">
                                <PenTool className="absolute left-4 top-4 text-brand-300" size={18} />
                                <textarea 
                                    className="w-full pl-12 p-4 bg-brand-50/50 border border-brand-100 rounded-2xl outline-none focus:ring-4 focus:ring-brand-900/5 focus:border-brand-900 transition-all font-medium text-sm min-h-[120px]" 
                        ...
                        <div className="border-t border-brand-100/50 dark:border-brand-800 pt-8">
                            <h4 className="text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-widest mb-6">Modèles d'emails</h4>
                            
                            <div className="space-y-6">
                                <div className="p-6 bg-white dark:bg-brand-800/50 rounded-[2rem] border border-brand-100 dark:border-brand-800">
                                    <div className="flex items-center gap-3 mb-4">
                                        <FileText size={18} className="text-blue-500" />
                                        <h5 className="font-black text-brand-900 dark:text-white">Envoi de Facture</h5>
                                    </div>
                                    <div className="space-y-4">
                                        <div>
                                            <label className="block text-[10px] font-bold text-brand-400 uppercase tracking-widest mb-2">Objet</label>
                                            <input 
                                                type="text" 
                                                ...
                                                value={userProfile.preferences?.invoiceEmailSubject || 'Votre facture {{invoiceNumber}}'}
                                                onChange={(e) => handlePreferenceChange('invoiceEmailSubject', e.target.value)}
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-[10px] font-bold text-brand-400 uppercase tracking-widest mb-2">Message</label>
                                            <textarea 
                                                className="w-full p-3 bg-brand-50/50 border border-brand-100 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm min-h-[100px]" 
                                                value={userProfile.preferences?.invoiceEmailTemplate || 'Bonjour,\n\nVeuillez trouver ci-joint la facture {{invoiceNumber}} d\'un montant de {{total}}.\n\nMerci de votre confiance.'}
                                                onChange={(e) => handlePreferenceChange('invoiceEmailTemplate', e.target.value)}
                                            />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="p-6 bg-white dark:bg-brand-800/50 rounded-[2rem] border border-brand-100 dark:border-brand-800">
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <h5 className="font-black text-brand-900 dark:text-white">Envoi de Devis</h5>
                                    </div>
                                    <div className="space-y-4">
                                        <div>
                                            <label className="block text-[10px] font-bold text-brand-400 uppercase tracking-widest mb-2">Objet</label>
                                            <input 
                                                ...
                                                value={userProfile.preferences?.quoteEmailSubject || 'Votre devis {{quoteNumber}}'}
                                                onChange={(e) => handlePreferenceChange('quoteEmailSubject', e.target.value)}
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-[10px] font-bold text-brand-400 uppercase tracking-widest mb-2">Message</label>
                                            <textarea 
                                                className="w-full p-3 bg-brand-50/50 border border-brand-100 rounded-xl outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500 transition-all text-sm min-h-[100px]" 
                                                value={userProfile.preferences?.quoteEmailTemplate || 'Bonjour,\n\nVeuillez trouver ci-joint le devis {{quoteNumber}} d\'un montant de {{total}}.\n\nJe reste à votre disposition pour toute question.'}
                                                onChange={(e) => handlePreferenceChange('quoteEmailTemplate', e.target.value)}
                                            />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="p-6 bg-white dark:bg-brand-800/50 rounded-[2rem] border border-brand-100 dark:border-brand-800">
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <h5 className="font-black text-brand-900 dark:text-white">Relance d'impayé</h5>
                                    </div>
                                    <div className="space-y-4">
                                        <div>
                                            <label className="block text-[10px] font-bold text-brand-400 uppercase tracking-widest mb-2">Objet</label>
                                            <input 
                                                ...
                                                value={userProfile.preferences?.reminderEmailSubject || 'Relance : Facture {{invoiceNumber}} impayée'}
                                                onChange={(e) => handlePreferenceChange('reminderEmailSubject', e.target.value)}
                                            />
                                        </div>
                                        <div>
                                            <label className="block text-[10px] font-bold text-brand-400 uppercase tracking-widest mb-2">Message</label>
                                            <textarea 
                                                className="w-full p-3 bg-brand-50/50 border border-brand-100 rounded-xl outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-sm min-h-[100px]" 
                                                value={userProfile.preferences?.reminderEmailTemplate || 'Bonjour,\n\nSauf erreur ou omission de notre part, la facture {{invoiceNumber}} d\'un montant de {{total}} arrivée à échéance le {{dueDate}} reste impayée.\n\nMerci de procéder au règlement dans les plus brefs délais.'}
                                                onChange={(e) => handlePreferenceChange('reminderEmailTemplate', e.target.value)}
                                            />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <p className="text-[10px] text-brand-400 font-medium italic">
                                    Variables disponibles : {'{{invoiceNumber}}'}, {'{{total}}'}, {'{{dueDate}}'}, {'{{clientName}}'}
                                </p>
                            </div>
                        </div>
                    </div>
              ...
              <div className="space-y-8 animate-slide-up">
                <div className="bg-pastel-pink/30 dark:bg-brand-900 rounded-[2.5rem] p-8 shadow-xl shadow-brand-900/5 border border-white/50 dark:border-brand-800 backdrop-blur-sm">
                    <div className="flex items-center gap-3 mb-8 border-b border-brand-100/50 dark:border-brand-800 pb-6">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-brand-900 dark:text-white font-display tracking-tight">Assistant IA & API</h3>
                            <p className="text-[10px] text-brand-500 font-bold uppercase tracking-widest mt-0.5">Connectez vos services externes</p>
                        </div>
                    </div>

                    <div className="space-y-8">
                        <div>
                            <label className="block text-[10px] font-bold text-brand-400 uppercase tracking-widest mb-2">Clé API Google Gemini</label>
                            <div className="relative">
                                <input 
                                    type="password" 
                                    className="w-full p-4 bg-brand-50/50 border border-brand-100 rounded-2xl outline-none focus:ring-4 focus:ring-brand-900/5 focus:border-brand-900 transition-all font-mono text-sm" 
                            ...
                            <p className="text-[10px] text-brand-400 mt-2 font-medium">
                                Nécessaire pour utiliser l'Assistant IA. Obtenez votre clé sur <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer" className="text-blue-500 hover:underline">Google AI Studio</a>.
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <div className="xl:col-span-1">
            <div className="sticky top-10">
                <div className="flex items-center justify-between mb-6 px-4">
                    <h4 className="text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-[0.3em]">Aperçu Facture</h4>
                ...
                <div className="bg-white dark:bg-brand-900 p-10 rounded-[3rem] shadow-2xl shadow-brand-900/10 border border-brand-100 dark:border-brand-800 min-h-[600px] flex flex-col relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-40 h-40 rounded-bl-[6rem] -mr-12 -mt-12 opacity-10 dark:opacity-20 blur-2xl group-hover:scale-110 transition-transform duration-700" style={{ backgroundColor: userProfile.logoColor || '#102a43' }}></div>
                    ...
                    <div className="border-b border-brand-50 dark:border-brand-800 pb-10 mb-10 relative z-10">
                        {userProfile.logoUrl ? (
                            <div className="mb-8 h-16 flex items-center justify-start">
                                <img src={userProfile.logoUrl} alt="Logo" className="max-h-full max-w-[200px] object-contain" />
                            </div>
                        ) : (
                            <div className="w-16 h-16 rounded-[1.25rem] flex items-center justify-center text-white mb-8 shadow-2xl transform -rotate-3 group-hover:rotate-0 transition-transform duration-500" style={{ backgroundColor: userProfile.logoColor || '#102a43', boxShadow: `0 20px 40px -10px ${userProfile.logoColor || '#102a43'}60` }}>
                                 <span className="font-black text-3xl">{userProfile.companyName.charAt(0)}</span>
                            </div>
                        )}
                        <h2 className="font-black text-brand-900 dark:text-white text-2xl leading-tight mb-2 font-display tracking-tighter">{userProfile.companyName || 'Votre Entreprise'}</h2>
                        {userProfile.professionalTitle && <p className="text-[10px] font-black text-brand-400 dark:text-brand-500 uppercase tracking-[0.2em] mb-4">{userProfile.professionalTitle}</p>}
                        
                        <div className="space-y-2 mt-6">
                            <div className="flex items-center gap-3 text-xs text-brand-500 dark:text-brand-400 font-medium">
                                <div className="p-1.5 bg-brand-50 dark:bg-brand-800 rounded-lg text-brand-300 dark:text-brand-600"><Mail size={12} /></div> {userProfile.email || 'email@exemple.com'}
                            </div>
                            <div className="flex items-center gap-3 text-xs text-brand-500 dark:text-brand-400 font-medium">
                                <div className="p-1.5 bg-brand-50 dark:bg-brand-800 rounded-lg text-brand-300 dark:text-brand-600"><Phone size={12} /></div> {userProfile.phone || '01 02 03 04 05'}
                            </div>
                            {userProfile.website && (
                                <div className="flex items-center gap-3 text-xs font-black" style={{ color: userProfile.logoColor || '#102a43' }}>
                                    <div className="p-1.5 bg-brand-50 dark:bg-brand-800 rounded-lg opacity-50" style={{ color: userProfile.logoColor || '#102a43' }}><Globe size={12} /></div> {userProfile.website}
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="space-y-6 opacity-20 dark:opacity-10 mb-auto">
                        ...
                        <div className="h-40 bg-brand-50/50 dark:bg-brand-800/30 rounded-[2rem] w-full border-2 border-brand-100 dark:border-brand-800 border-dashed"></div>
                        <div className="space-y-4">
                    ...
                    <div className="mt-12 pt-8 border-t border-brand-50 dark:border-brand-800 text-[10px] text-center text-brand-400 dark:text-brand-500 leading-relaxed">
                         <p className="font-black text-brand-900 dark:text-white mb-2 uppercase tracking-[0.2em]">{userProfile.companyName}</p>
                         <p className="max-w-[240px] mx-auto font-medium">{userProfile.address}</p>
                         <div className="flex items-center justify-center gap-4 mt-4 font-black text-brand-500 dark:text-brand-400">
                            <span className="px-2 py-1 bg-brand-50 dark:bg-brand-800 rounded-md">SIRET: {userProfile.siret}</span>
                            {userProfile.tvaNumber && <span className="px-2 py-1 bg-brand-50 dark:bg-brand-800 rounded-md">TVA: {userProfile.tvaNumber}</span>}
                         </div>
                         {!userProfile.tvaNumber && <p className="mt-3 italic font-medium">TVA non applicable, art. 293 B du CGI</p>}
                         {userProfile.legalMentions && <p className="mt-4 text-[9px] opacity-60 italic leading-tight font-medium">{userProfile.legalMentions}</p>}
                    </div>
                </div>
                
                <div className="mt-8 flex items-center justify-center gap-4 text-blue-600 dark:text-blue-400 bg-white dark:bg-brand-900 p-5 rounded-[1.5rem] border border-brand-100 dark:border-brand-800 text-[10px] font-black uppercase tracking-widest shadow-xl shadow-brand-900/5 transition-all">
                    {showSaveIndicator ? (
                        <>
                            <div className="p-1.5 bg-emerald-500 text-white rounded-full shadow-lg shadow-emerald-500/20">
                                <CheckCircle2 size={14} className="animate-pulse" />
                            </div>
                            <span className="text-emerald-600 dark:text-emerald-400">Modifications enregistrées</span>
                        </>
                    ) : (
                        <>
