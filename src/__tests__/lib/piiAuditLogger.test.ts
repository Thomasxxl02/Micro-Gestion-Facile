...
describe("PIIAuditLogger", () => {
  ...
  it("initialise correctement les logs depuis localStorage", () => {
    const mockEvent = { timestamp: Date.now(), patternType: "email", context: "test" };
  ...
  it("enregistre une détection et masque le texte", () => {
    PIIAuditLogger.logDetection("email", "secret@email.com", "[MASKED]", "email_context", "general");
    ...
    expect(report.recentEvents[0].detectedText).toBe("secret@email.com".substring(0, 20));
  });

  it("applique la rotation des logs (MAX_EVENTS = 1000)", () => {
    // Simuler 1005 événements
    for (let i = 0; i < 1005; i++) {
      PIIAuditLogger.logDetection("phoneFR", `06000000${i}`, "XXX", "test", "general");
    }
  ...
  it("enregistre les patterns inconnus et les limite", () => {
    PIIAuditLogger.logUnknownPattern("Long text that should be truncated because it might be a PII", "query");
