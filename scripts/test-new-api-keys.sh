...
if npm run build; then
    # Find the main bundle (index-*.js)
    MAIN_BUNDLE=$(ls dist/assets/index-*.js 2>/dev/null | head -n 1)
    if [ -f "$MAIN_BUNDLE" ]; then
        BUILD_OUTPUT=$(stat -f%z "$MAIN_BUNDLE" 2>/dev/null || stat -c%s "$MAIN_BUNDLE" 2>/dev/null || echo "unknown")
        print_success "Production build completed (bundle size: $BUILD_OUTPUT bytes)"
    else
        print_warning "Production build completed, but main bundle not found for size check."
    fi
else
