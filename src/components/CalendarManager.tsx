        ...
        <div className="lg:col-span-3 bg-white border border-brand-100 rounded-[2.5rem] shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="flex-1 grid grid-cols-7 border-b border-brand-50">
            {dayNames.map((day) => (
              <div
                key={day}
                className="py-4 text-center text-[10px] font-bold text-brand-400 uppercase tracking-[0.2em] border-r border-brand-50 last:border-0"
              >
                {day}
              </div>
            ))}
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              {events
                ...
                .map((ev) => (
                  <button
                    key={ev.id}
                    className="p-4 bg-brand-50 rounded-2xl border border-brand-100 hover:border-brand-300 transition-all group cursor-pointer text-left w-full"
                    onClick={() => {
                      setSelectedEvent(ev);
                      setFormData(ev);
                      setIsModalOpen(true);
                  ...
                  >
                    ...
                    <h4 className="text-sm font-bold text-brand-900 mb-1 group-hover:text-brand-700 transition-colors">
                      {ev.title}
                    </h4>
          ...
          <div className="bg-brand-900 text-white rounded-4xl p-8 shadow-xl shadow-brand-900/10">
            ...
            <p className="text-xs text-brand-200 leading-relaxed">
              Liez vos événements à des clients ou des factures pour garder une trace de vos
