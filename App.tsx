...
import { ViewState, Invoice, Client, UserProfile, Supplier, Product, Expense, Email, EmailTemplate, CalendarEvent, Theme } from './types';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--bg-main)] p-6 transition-colors duration-500">
        ...
        <div className="max-w-md w-full bg-[var(--card-bg)] p-10 rounded-[2.5rem] shadow-2xl text-center border border-[var(--card-border)] relative z-10 animate-fade-in">
          ...
          <h1 className="text-4xl font-bold text-[var(--text-main)] mb-4 tracking-tight">Micro Gestion</h1>
          <p className="text-[var(--text-muted)] mb-12 text-lg">Votre allié quotidien pour une gestion simplifiée et intelligente.</p>
          ...
          <p className="mt-10 text-xs text-[var(--text-muted)] uppercase tracking-widest font-semibold">Sécurisé par Google Auth</p>
        </div>
      </div>
    );
  }
      ...
      <main className="flex-1 lg:ml-64 p-4 lg:p-8 xl:p-12 overflow-x-hidden relative z-10">
        ...
        <div className="lg:hidden flex justify-between items-center mb-8 sticky top-0 bg-[var(--bg-main)]/60 backdrop-blur-xl z-30 py-4 border-b border-[var(--card-border)] px-4 -mx-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-black text-[var(--text-main)] tracking-tighter">MICRO<span className="text-brand-500">GESTION</span></h1>
          </div>
        ...
        <div className="hidden lg:flex justify-end items-center gap-6 mb-10 animate-fade-in">
           <div className="flex items-center gap-4 px-4 py-2 bg-[var(--card-bg)]/50 backdrop-blur-sm rounded-2xl border border-[var(--card-border)] shadow-sm">
             <div className="flex flex-col items-end">
               <span className="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-wider">Connecté en tant que</span>
               <span className="text-sm font-bold text-[var(--text-main)]">{user.displayName || user.email}</span>
             </div>
             <div className="w-10 h-10 rounded-xl bg-[var(--bg-main)] flex items-center justify-center overflow-hidden border border-[var(--card-border)]">
               {user.photoURL ? (
                 ...
                 <span className="text-[var(--text-main)] font-bold">{user.email?.[0].toUpperCase()}</span>
               )}
             </div>
