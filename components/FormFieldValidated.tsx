...
import { useFieldValidation } from './useFormValidation';
...
// Map des validateurs
const validatorsMap: Record<ValidationType, (value: any) => ValidationResult> = {
  siret: validateSIRET,
  siren: validateSIREN,
  iban: validateIBAN,
  email: validateEmail,
...
export const FormFieldValidated: React.FC<FormFieldValidatedProps> = ({
  ...
  const determineValidator = (): ((value: any) => ValidationResult) => {
  ...
  // Hook de validation
  const { error, isValid, handleChange: handleValidationChange } = useFieldValidation(
    value,
    finalValidator,
    { validateOnChange },
  );
  ...
  const ariaAttrs = {
    ...
    'aria-invalid': !isValid ? ('true' as const) : ('false' as const),
  };
  ...
  return (
    <div className={`space-y-1.5 ${className}`}>
      ...
      <div className="relative">
        ...
        {isTextarea ? (
          <textarea
            ...
            maxLength={maxLength}
            className={`w-full ${Icon ? 'pl-12' : 'pl-4'} p-4 bg-brand-50/50 border border-brand-100 rounded-2xl outline-none focus:ring-4 focus:ring-brand-900/5 focus:border-brand-900 transition-all resize-y min-h-24 ${
              hasError
          ...
          <input
            ...
            maxLength={maxLength}
            className={`w-full ${Icon ? 'pl-12' : 'pl-4'} p-4 bg-brand-50/50 border border-brand-100 rounded-2xl outline-none focus:ring-4 focus:ring-brand-900/5 focus:border-brand-900 transition-all ${
              hasError
                ? 'border-red-500 focus:ring-red-500/10 focus:border-red-600'
                : hasSuccess
                  ? 'border-green-500 focus:ring-green-500/10 focus:border-green-600'
      ...
      {/* Erreur */}
      {hasError && (
        <p id={`${fieldId}-error`} className="text-[10px] text-red-600 font-medium flex items-center gap-1">
...
export const SIRETField: React.FC<Omit<FormFieldValidatedProps, 'validationType'>> = (props) => (
  <FormFieldValidated {...props} validationType="siret" placeholder="12345678901234" maxLength={14} />
);
