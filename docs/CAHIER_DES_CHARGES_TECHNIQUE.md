...
interface Product {
  ...
  stockUnit: 'unité' | 'kg' | 'litre' | 'm' | 'm²' | 'heure' | 'jour' | 'forfait';
...
function computeMargin(sellPriceHT: number, purchasePriceHT: number): number {
  ...
  return sell.minus(purchase).dividedBy(sell).times(100).toDecimalPlaces(2).toNumber();
}
```
...
class MicroGestionDB extends Dexie {
  ...
  constructor() {
    ...
    this.version(8).stores({
      ...
      emails: '++id, threadId, direction, linkedClientId, sentAt, receivedAt, status',
