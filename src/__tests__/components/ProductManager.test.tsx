...
describe("ProductManager Component", () => {
  ...
  describe("Rendering & UI", () => {
    it("rend le gestionnaire de produits", () => {
      ...
      expect(
        screen.queryByText(/Produits|Products/i) ??
          screen.queryByText(/Package/i),
      ).toBeDefined();
    });
    ...
    it("affiche les catégories de produits", () => {
      ...
      expect(
        servicesElements.length > 0 || fournituresElements.length > 0,
      ).toBe(true);
    });
  });
  ...
  describe("Sorting", () => {
    ...
    it("trie par prix", async () => {
      ...
      const sortButtons = screen
        ...
        .filter(
          (b) =>
            b.textContent?.includes("Prix") ||
            b.textContent?.toLowerCase().includes("price"),
        );
  ...
  describe("Unit & SKU Management", () => {
    it("affiche le SKU du produit", () => {
      ...
      expect(
        screen.queryByText("CONS-001") ?? screen.queryByText("MAINT-001"),
      ).toBeDefined();
    });
  ...
  describe("Empty State", () => {
    it("affiche un message si pas de produits", () => {
      render(
        <ProductManager products={[]} onSave={onSave} onDelete={onDelete} />,
      );
