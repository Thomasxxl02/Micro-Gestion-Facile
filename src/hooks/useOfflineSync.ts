...
import { getDexieTable, type DexieTableName } from "../db/repositories";
...
export function useOfflineSync<
  T extends { id: string; updatedAt?: string | number },
>(options: UseOfflineSyncOptions) {
  ...
  const offlinePriority = useUIStore((state) => state.offlinePriority);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const localData = (await table.toArray()) as T[];
        setData(localData);
        setStatus("SUCCESS");
        setFromCache(true);
      } catch (err) {
        console.error(
          `[OfflineSync] Erreur lecture Dexie ${String(dexieTableName)}:`,
          err,
        );
      }
    })();
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        if (dexieTableName) {
          void (async () => {
            try {
              ...
              console.error(
                `[OfflineSync] Erreur sync Dexie ${String(dexieTableName)}:`,
                err,
              );
            }
          })();
        ...
        if (snapshot.metadata.fromCache) {
          if (import.meta.env.DEV) {
            console.warn(
              `[OfflineSync] ${collectionName} depuis cache local (offline détecté)`,
            );
          }
        }
        ...
        if (snapshot.metadata.hasPendingWrites) {
          if (import.meta.env.DEV) {
            console.warn(
              `[OfflineSync] ${collectionName}: modifications locales en attente`,
            );
          }
        }
      },
  ...
  const upsert = useCallback(
    async (item: T) => {
      ...
      try {
        ...
        // Mode Hors-ligne Prioritaire : On sauve en local et on laisse le background sync (ou on attend pas)
        if (offlinePriority) {
          // On ne "wait" pas le setDoc pour une réactivité maximale
          setDoc(docRef, dataToSave, { merge: true }).catch((err) => {
            console.error(
              `[OfflineSync] Erreur sync asynchrone Firestore ${collectionName}:`,
              err,
            );
          });
          return { success: true };
  ...
  const remove = useCallback(
    async (id: string) => {
      try {
        ...
        console.error(
          `[OfflineSync] Erreur suppression ${collectionName}:`,
          err,
        );
        return { success: false, error: err };
      }
