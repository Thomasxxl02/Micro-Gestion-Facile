                        ...
                        <div>
                            ...
                            {selectedClient && (
                                <div className="mt-4 p-4 bg-brand-50 rounded-xl border border-brand-100 text-sm">
                                    ...
                                    <p className="text-brand-500 whitespace-pre-line mt-1">{selectedClient.address}</p>
                                    {selectedClient.siret && <p className="text-xs text-brand-400 mt-2 font-mono">SIRET: {selectedClient.siret}</p>}
                                </div>
                            )}
                        </div>
                ...
                <div className="bg-white rounded-4xl shadow-sm border border-brand-200 p-8">
                     ...
                     <div className="space-y-4">
                        {newDocData.items?.map((item, index) => (
                            <div key={item.id} className="flex flex-col md:flex-row gap-4 items-start md:items-center bg-brand-50/50 p-4 rounded-xl border border-brand-100 group hover:border-accent-200 transition-colors">
                                            ...
                                            {products
                                                ...
                                                .map(p => (
                                                    <button
                                                        ...
                                                        className="w-full p-3 text-left hover:bg-brand-50 border-b border-brand-50 last:border-0 flex justify-between items-center group transition-colors"
                                                    >
                                                        <div>
                                                            <div className="text-xs font-bold text-brand-900 group-hover:text-accent-600">{p.name}</div>
                                                            <div className="text-[10px] text-brand-400 uppercase font-bold">{p.category || 'Sans catégorie'}</div>
                                                        </div>
                                                        <div className="text-right">
                                                            <div className="text-xs font-bold text-brand-900">{p.price.toFixed(2)}€</div>
                                                            {p.type === 'product' && <div className="text-[10px] text-brand-400">Stock: {p.stock}</div>}
                                                        </div>
                                                    </button>
                                                ))
                                            }
                                            {products.length === 0 && <div className="p-4 text-center text-xs text-brand-400 italic">Catalogue vide</div>}
                                        </div>
                                    </div>
                                )}
                            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="card-modern p-6 flex flex-col justify-between relative overflow-hidden group">
              ...
              <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="badge bg-brand-50 dark:bg-brand-800 text-brand-600 dark:text-brand-400">Total {getDocumentLabel(activeTab)}s</span>
              </div>
              <div>
                  <h3 className="text-2xl font-bold text-brand-900 dark:text-white font-display tracking-tight">{stats.total.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</h3>
                  <p className="text-xs text-brand-500 dark:text-brand-400 mt-1 font-medium">{stats.count} documents au total</p>
              </div>
          </div>
          <div className="card-modern p-6 flex flex-col justify-between relative overflow-hidden group">
              ...
              <div>
                  ...
                  <p className="text-xs text-brand-500 dark:text-brand-400 mt-1 font-medium">Revenus sécurisés</p>
              </div>
          </div>
          <div className="card-modern p-6 flex flex-col justify-between relative overflow-hidden group">
              ...
              <div>
                  ...
                  <p className="text-xs text-brand-500 dark:text-brand-400 mt-1 font-medium">À encaisser</p>
              </div>
          </div>
          <div className="card-modern p-6 flex flex-col justify-between relative overflow-hidden group border-red-100 dark:border-red-900/30">
              ...
              <div>
                  ...
                  <p className="text-xs text-brand-500 dark:text-brand-400 mt-1 font-medium">Action requise</p>
              </div>
          </div>
      </div>
