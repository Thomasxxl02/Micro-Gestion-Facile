 ...
 *
 * // Dans JSX:
 * <p>Le lien expire dans: <strong>{formattedTimeLeft}</strong></p>
 * <button disabled={!canResend} onClick={() => {
 *   resendEmail();
 *   startResendCooldown();
 * }}>
 *   {canResend ? 'Renvoyer' : `Renvoyer dans ${resendCooldownSeconds}s`}
 * </button>
 * ```
 */
