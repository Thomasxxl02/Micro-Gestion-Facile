...
const ClientManager: React.FC<ClientManagerProps> = ({ clients, setClients, invoices, onSave, onDelete }) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto relative pb-10">
      ...
      <div className="bento-grid">
          <div className="bento-item">
              <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-[10px] font-bold text-brand-400 uppercase tracking-widest">Clients Actifs</span>
              </div>
              <div>
                  <h3 className="text-2xl font-bold text-brand-900 dark:text-white font-display">{globalStats.count}</h3>
                  <p className="text-xs text-brand-500 dark:text-brand-400 mt-1">Portefeuille client actuel</p>
              </div>
          </div>
          <div className="bento-item">
              <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-[10px] font-bold text-brand-400 uppercase tracking-widest">CA Total Encaissé</span>
              </div>
              <div>
                  ...
                  <p className="text-xs text-brand-500 dark:text-brand-400 mt-1">Revenu cumulé</p>
              </div>
          </div>
          <div className="bento-item">
              <div className="flex justify-between items-start mb-4">
                  ...
                  <span className="text-[10px] font-bold text-brand-400 uppercase tracking-widest">Clients Archivés</span>
              </div>
              <div>
                  <h3 className="text-2xl font-bold text-brand-900 dark:text-white font-display">{globalStats.archivedCount}</h3>
                  <p className="text-xs text-brand-500 dark:text-brand-400 mt-1">Clients inactifs</p>
              </div>
          </div>
      ...
      {/* Side Panel Form */}
      <div className={`fixed inset-y-0 right-0 w-full sm:w-[500px] bg-white shadow-2xl transform transition-transform duration-500 ease-in-out z-50 border-l border-brand-100 ${isPanelOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-full flex flex-col">
            <div className="p-8 border-b border-brand-50 flex justify-between items-center bg-brand-50/30">
                <div>
                    <h3 className="text-xl font-bold text-brand-900 font-display">{editingId ? 'Modifier le client' : 'Nouveau client'}</h3>
                    <p className="text-xs text-brand-400 mt-1">Remplissez les informations détaillées ci-dessous.</p>
                </div>
                <button onClick={() => setIsPanelOpen(false)} className="p-2.5 hover:bg-brand-100 rounded-xl text-brand-400 transition-all">
                    <X size={20} />
                </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                <div className="space-y-6">
                    <div className="text-[10px] font-bold text-brand-400 uppercase tracking-[0.2em] border-b border-brand-50 pb-2">Informations Générales</div>
                    <div>
                        <label className="block text-[10px] font-bold text-brand-500 uppercase tracking-wider mb-2">Nom / Société <span className="text-red-500">*</span></label>
                        <input 
                            type="text" 
                            required
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {processedClients.map(client => {
            ...
            <div 
                key={client.id} 
                className={`card-modern p-8 group relative flex flex-col ${client.archived ? 'grayscale-[0.5] opacity-80' : ''}`}
            >
               {/* Actions Top Right */}
